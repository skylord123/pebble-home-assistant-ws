...
function initEntityPicker() {
	...
	if (!watch_config || !watch_config['all_entities'] || !watch_config['all_entities'].length) {
		listEl.html('<div style="padding:0.5rem;color:var(--color-text-muted);">-- No entities loaded --</div>');
		status.text('Bridge not available or entity fetch failed. Check the WebView console.');
		return;
	}
	...
	function addToList(listId, listKey, removeClass) {
		if (!selectedEntity) {
		...
		watch_config[listKey].push({ entity_id: entityId, name: name });
		...
		const li = $(
			'<li class="list-group-item" data-entity-id="' + entityId + '" data-friendly-name="' + (name || '').replace(/"/g, '&quot;') + '">' +
	...
	btnFav.on('click', function () {
		addToList('#favorites-sortable', 'favorite_entities', 'remove-favorite');
